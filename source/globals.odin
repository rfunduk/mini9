...
sym: struct {
	...
	holes, interval, kinematic:                 mrb.Value,
	layer, leading, lifetime:                   mrb.Value,
	limit, line, loop, margin, mask, max:       mrb.Value,
	mode, name, offset, origin, outline:        mrb.Value,
	...
	spacing, spin, start, states, static:       mrb.Value,
...
cache_symbols :: proc() {
	...
	sym = {
		_handler_for = sv(intern(s, "_handler_for")),
		...
		kinematic    = sv(intern(s, "kinematic")),
