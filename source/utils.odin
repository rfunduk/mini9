...
cache_symbols :: proc() {
	s := g.mrb_state
	sym :: mrb.symbol_value
	intern :: mrb.intern_cstr
	g.sym = {
		align       = sym(intern(s, "align")),
		atlas       = sym(intern(s, "atlas")),
		body        = sym(intern(s, "body")),
		clip        = sym(intern(s, "clip")),
		color       = sym(intern(s, "color")),
		default     = sym(intern(s, "default")),
		delay       = sym(intern(s, "delay")),
		density     = sym(intern(s, "density")),
		direction   = sym(intern(s, "direction")),
		easing      = sym(intern(s, "easing")),
		enter       = sym(intern(s, "enter")),
		exit        = sym(intern(s, "exit")),
		fade_in     = sym(intern(s, "fade_in")),
		fade_out    = sym(intern(s, "fade_out")),
		filled      = sym(intern(s, "filled")),
		fliph       = sym(intern(s, "fliph")),
		flipv       = sym(intern(s, "flipv")),
		font        = sym(intern(s, "font")),
		frame       = sym(intern(s, "frame")),
		frames      = sym(intern(s, "frames")),
		friction    = sym(intern(s, "friction")),
		interval    = sym(intern(s, "interval")),
		layer       = sym(intern(s, "layer")),
		loop        = sym(intern(s, "loop")),
		mask        = sym(intern(s, "mask")),
		mode        = sym(intern(s, "mode")),
		offset      = sym(intern(s, "offset")),
		outline     = sym(intern(s, "outline")),
		pitch       = sym(intern(s, "pitch")),
		polyphony   = sym(intern(s, "polyphony")),
		pos         = sym(intern(s, "pos")),
		restitution = sym(intern(s, "restitution")),
		rotation    = sym(intern(s, "rotation")),
		rounded     = sym(intern(s, "rounded")),
		scale       = sym(intern(s, "scale")),
		sensor      = sym(intern(s, "sensor")),
		size        = sym(intern(s, "size")),
		slide       = sym(intern(s, "slide")),
		spacing     = sym(intern(s, "spacing")),
		states      = sym(intern(s, "states")),
		target      = sym(intern(s, "target")),
		thickness   = sym(intern(s, "thickness")),
		update      = sym(intern(s, "update")),
		values      = sym(intern(s, "values")),
		visible     = sym(intern(s, "visible")),
		volume      = sym(intern(s, "volume")),
		wrap        = sym(intern(s, "wrap")),
		zoom        = sym(intern(s, "zoom")),
	}
}
