...
RAYLIB_WASM_LIB :: #config(RAYLIB_WASM_LIB, "")
...
when ODIN_OS == .Darwin {
	...
	foreign import lib {RAYLIB_WASM_LIB}
} else {
...
foreign lib {
	...
	// Shaders management [v6 rename pass]
	LoadShader :: proc(code: cstring, type: c.int) -> c.uint --- // Load (compile) shader, return shader id [v6: was CompileShader]
	LoadShaderProgram :: proc(vsCode, fsCode: cstring) -> c.uint --- // Load shader from code strings [v6: was LoadShaderCode]
	LoadShaderProgramEx :: proc(vsId, fsId: c.uint) -> c.uint --- // Load shader program using already loaded shader ids [v6: was LoadShaderProgram(vShaderId, fShaderId)]
	LoadShaderProgramCompute :: proc(csId: c.uint) -> c.uint --- // Load compute shader program [v6 new]
	UnloadShader :: proc(id: c.uint) --- // Unload shader, loaded with LoadShader() [v6 new]
	...
	// Compute shader management [v6: LoadComputeShaderProgram moved to LoadShaderProgramCompute above]
